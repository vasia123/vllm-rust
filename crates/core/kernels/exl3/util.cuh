...
// cublas helpers stripped (host-only, used by training-side launchers
// we don't compile). If a vendored .cu references `cublasStatus_t` or
// `cublas_check`, that .cu must be edited to drop the host launcher.
...
/* cublas helpers stripped — see top-of-file note. */
