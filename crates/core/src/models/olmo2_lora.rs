//! OLMo2 model with LoRA adapter support.
//!
//! OLMo2 differs from standard pre-norm transformers (Llama, Qwen) in two key ways:
//! 1. POST-normalization: RMSNorm is applied after the residual addition, not before.
//! 2. QK normalization: Per-head RMSNorm on Q and K projections before RoPE.
//!
...
// ─── Attention with LoRA ─────────────────────────────────────────────────────
//
// OLMo2 uses QK normalization: per-head RMSNorm on Q and K before RoPE.

struct Olmo2AttentionWithLora {
...
// not before the sublayer (unlike pre-norm transformers like Llama/Qwen3).

struct Olmo2DecoderLayerWithLora {
    self_attn: Olmo2AttentionWithLora,
    mlp: Olmo2MlpWithLora,
    post_attention_layernorm: RmsNorm,
    post_feedforward_layernorm: RmsNorm,
}

impl Olmo2DecoderLayerWithLora {
    fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        let self_attn = Olmo2AttentionWithLora::new(cfg, vb.pp("self_attn"))?;
        let mlp = Olmo2MlpWithLora::new(cfg.hidden_size, cfg.intermediate_size, vb.pp("mlp"))?;
    ...
    fn forward(
        ...
        let residual = &xs;
        let mlp_out = self.mlp.forward(&xs, lora_ctx)?;
...
/// OLMo2 model with LoRA adapter support.
///
/// Uses post-normalization (norms after residual addition) and per-head QK
/// normalization. No embedding scaling. SwiGLU MLP activation.
pub struct Olmo2WithLora {
    embed_tokens: Embedding,
    layers: Vec<Olmo2DecoderLayerWithLora>,
    norm: RmsNorm,
...
impl Olmo2WithLora {
    /// Create a new OLMo2 model with LoRA support.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(Olmo2DecoderLayerWithLora::new(cfg, vb_l.pp(i))?);
        }
...
impl crate::engine::ModelForward for Olmo2WithLora {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
impl crate::models::ModelForwardWithLora for Olmo2WithLora {
    fn register_lora(&mut self, lora_model: &LoraModel) {
        Olmo2WithLora::register_lora(self, lora_model)
    }
...
mod tests {
    ...
    #[test]
    fn test_olmo2_with_lora_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = Olmo2WithLora::new(&cfg, vb);
        assert!(model.is_ok(), "Olmo2WithLora should construct");
        assert_eq!(model.unwrap().layers.len(), cfg.num_hidden_layers);
    }

    #[test]
    fn test_olmo2_with_lora_forward_no_adapter() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Olmo2WithLora::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_olmo2_with_lora_register_and_forward() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let mut model = Olmo2WithLora::new(&cfg, vb).unwrap();
