//! Mistral model with LoRA adapter support.
//!
//! Architecturally identical to the Llama LoRA variant with the addition of
//! sliding window attention support. LoRA is applied to all attention
//! projections (q, k, v, o) and MLP projections (gate, up, down).
...
struct MistralAttentionWithLora {
...
struct MistralDecoderLayerWithLora {
    self_attn: MistralAttentionWithLora,
    mlp: SwiGluMlpWithLora,
    input_layernorm: RmsNorm,
    post_attention_layernorm: RmsNorm,
}

impl MistralDecoderLayerWithLora {
    fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        let self_attn = MistralAttentionWithLora::new(cfg, vb.pp("self_attn"))?;
...
/// Mistral model with LoRA adapter support.
///
/// This is a variant of MistralForCausalLM that uses LinearWithLora layers,
/// enabling per-request adapter selection. Supports sliding window attention.
pub struct MistralWithLora {
    embed_tokens: Embedding,
    layers: Vec<MistralDecoderLayerWithLora>,
    norm: RmsNorm,
...
impl MistralWithLora {
    /// Create a new Mistral model with LoRA support.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(MistralDecoderLayerWithLora::new(cfg, vb_l.pp(i))?);
        }
...
impl crate::engine::ModelForward for MistralWithLora {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
impl crate::models::ModelForwardWithLora for MistralWithLora {
    fn register_lora(&mut self, lora_model: &LoraModel) {
        MistralWithLora::register_lora(self, lora_model)
    }
...
mod tests {
    ...
    #[test]
    fn test_mistral_with_lora_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = MistralWithLora::new(&cfg, vb);
        assert!(model.is_ok(), "MistralWithLora should construct");
        assert_eq!(model.unwrap().layers.len(), cfg.num_hidden_layers);
    }

    #[test]
    fn test_mistral_with_lora_forward_no_adapter() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = MistralWithLora::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_mistral_with_lora_register_and_forward() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let mut model = MistralWithLora::new(&cfg, vb).unwrap();
