...
//!
//! GlmForCausalLM is a thin wrapper over Llama architecture with:
//! - partial_rotary_factor = 0.5 (only half of head_dim is rotated)
//! - is_neox_style = false (split rotation instead of interleaved)
//! - No bias on o_proj in attention
//!
//! Reference: vLLM's glm.py implementation.
...
/// GLM uses half partial rotary factor.
const GLM_PARTIAL_ROTARY_FACTOR: f64 = 0.5;
/// GLM uses split rotation style (not neox interleaved).
...
impl GlmAttention {
    fn new_with_tp(cfg: &ModelConfig, vb: VarBuilder, pg: &dyn ProcessGroup) -> Result<Self> {
        ...
        // GLM uses no bias on all projections
        ...
        // GLM-specific rotary embedding: partial factor 0.5, split style
    ...
    fn forward(
        ...
        // Apply partial RoPE (GLM-specific: only rotates half of head_dim)
...
impl GlmDecoderLayer {
    fn new_with_tp(cfg: &ModelConfig, vb: VarBuilder, pg: &dyn ProcessGroup) -> Result<Self> {
        let self_attn = GlmAttention::new_with_tp(cfg, vb.pp("self_attn"), pg)?;
...
/// GLM model for causal language modeling.
///
/// This is a Llama-like architecture with:
/// - Partial rotary embedding (factor 0.5)
/// - Split rotation style (not neox interleaved)
/// - No bias on o_proj
pub struct GlmForCausalLM {
...
impl GlmForCausalLM {
    /// Create a new GLM model for single GPU.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        Self::new_with_tp(cfg, vb, &LocalProcessGroup::new(), TpContext::single_gpu())
    }

    /// Create a new GLM model with tensor parallelism.
    ///
    /// # Arguments
    ...
    pub fn new_with_tp(
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(GlmDecoderLayer::new_with_tp(cfg, vb_l.pp(i), pg)?);
        }
...
impl crate::engine::ModelForward for GlmForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    #[test]
    fn test_glm_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = GlmForCausalLM::new(&cfg, vb);
        assert!(model.is_ok(), "GlmForCausalLM should construct");
    ...
    #[test]
    fn test_glm_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = GlmForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_glm_prefill_then_decode() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = GlmForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_glm_tp_construction_world_size_2() {
        ...
        let model = GlmForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx);
        assert!(
            model.is_ok(),
            "GlmForCausalLM should construct with TP=2: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_glm_tp_forward_world_size_2() {
        let cfg = test_config_tp_compatible();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let model = GlmForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx).expect("build model");
    ...
    #[test]
    fn test_glm_tp_heads_divisibility_check() {
        ...
        let result = GlmForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx);
