//! Quantized Mistral model implementation.
//!
//! This module provides a quantized version of the Mistral model that supports
//! various quantization methods (FP8, GPTQ, AWQ) through the QuantizedWeightLoader
//! abstraction.
//!
//! Mistral is architecturally identical to Llama (SwiGLU, RMSNorm, GQA).
//! Sliding window attention is handled at the engine level, not in the
...
struct QuantizedMistralAttention {
...
impl QuantizedMistralAttention {
...
struct QuantizedMistralDecoderLayer {
    self_attn: QuantizedMistralAttention,
    mlp: QuantizedSwiGluMlp,
    input_layernorm: RmsNorm,
    post_attention_layernorm: RmsNorm,
}

impl QuantizedMistralDecoderLayer {
    fn new(
        cfg: &ModelConfig,
        loader: &dyn QuantizedWeightLoader,
        vb: VarBuilder,
        ...
        let self_attn =
            QuantizedMistralAttention::new(cfg, loader, &format!("{prefix}.self_attn"))?;
...
/// Quantized Mistral model supporting FP8, GPTQ, AWQ, and unquantized weights.
pub struct QuantizedMistralForCausalLM {
    embed_tokens: Embedding,
    layers: Vec<QuantizedMistralDecoderLayer>,
    norm: RmsNorm,
...
impl QuantizedMistralForCausalLM {
    pub fn new(
        cfg: &ModelConfig,
        vb: VarBuilder,
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(QuantizedMistralDecoderLayer::new(
                cfg,
                weight_loader,
                vb.clone(),
                i,
...
impl crate::engine::ModelForward for QuantizedMistralForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    #[test]
    fn test_quantized_mistral_construction() {
        let cfg = test_config();
        ...
        let model = QuantizedMistralForCausalLM::new(&cfg, vb, loader.as_ref());
        assert!(
            model.is_ok(),
            "QuantizedMistralForCausalLM should construct with unquantized loader"
        );
    ...
    #[test]
    fn test_quantized_mistral_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let loader = create_weight_loader_with_params(vb.clone(), &detected);
        let model =
            QuantizedMistralForCausalLM::new(&cfg, vb, loader.as_ref()).expect("build model");
