...
/// Extract Granite-specific config from the extra map.
...
// Granite = vanilla GQA + bias on ALL projections (toggle, default false) +
// custom attention scaling via cfg.extra.attention_multiplier (default
// 1/sqrt(head_dim)).
//
// NOTE: the bespoke implementation only applied attention_multiplier in the
// CUDA batched-decode path; the prefill and CPU-decode paths used the default
// 1/sqrt(head_dim) scale. This migration applies the multiplier consistently
// across all paths — fixing that latent inconsistency. When
// attention_multiplier == 1/sqrt(head_dim) (the default), behavior is
...
impl GraniteAttention {
    fn new_with_tp(
        ...
        .with_bias(bias);
        // Only force the manual path (with custom scale) when the multiplier
        // actually differs from the default 1/sqrt(head_dim). For the default
        // case the fast paged_attention path already uses the same scale.
        let default_scale = 1.0 / (cfg.head_dim as f64).sqrt();
        if (granite_cfg.attention_multiplier - default_scale).abs() > f64::EPSILON {
            attn_cfg = attn_cfg.with_scale(granite_cfg.attention_multiplier);
        }
...
impl GraniteDecoderLayer {
    fn new_with_tp(
        ...
        Ok(Self {
            ...
            residual_multiplier: granite_cfg.residual_multiplier,
        })
    }
    ...
    fn forward(
        ...
        // Granite: residual + hidden * residual_multiplier
...
impl GraniteForCausalLM {
    ...
    pub fn new_with_tp(
        ...
        let granite_cfg = GraniteConfig::from_model_config(cfg);
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(GraniteDecoderLayer::new_with_tp(
                cfg,
                &granite_cfg,
                vb_l.pp(i),
                pg,
        ...
        // Logit scale: 1.0 / logits_scaling (to match Python vLLM pattern)
        let logit_scale = cfg
            .extra
            .get("logit_scale")
            .and_then(|v| v.as_f64())
            .unwrap_or(1.0)
            / granite_cfg.logits_scaling;
        ...
        Ok(Self {
            ...
            dtype: vb.dtype(),
            embedding_multiplier: granite_cfg.embedding_multiplier,
            logit_scale,
        })
    }
...
impl crate::engine::ModelForward for GraniteForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    fn test_granite_construction() {
        ...
        let model = GraniteForCausalLM::new(&cfg, vb);
        assert!(
            model.is_ok(),
            "GraniteForCausalLM should construct: {:?}",
            model.err()
        );
    ...
    fn test_granite_forward_shape() {
        ...
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = GraniteForCausalLM::new(&cfg, vb).expect("build model");
    ...
    fn test_granite_config_defaults() {
        ...
        let gc = GraniteConfig::from_model_config(&cfg);
        assert!((gc.residual_multiplier - 1.0).abs() < f64::EPSILON);
        assert!((gc.embedding_multiplier - 1.0).abs() < f64::EPSILON);
        assert!((gc.logits_scaling - 1.0).abs() < f64::EPSILON);
    }
}
