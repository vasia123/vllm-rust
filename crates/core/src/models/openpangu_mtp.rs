//! OpenPangu Multi-Token Prediction (MTP) draft model.
//!
//! OpenPangu MTP uses the standard DeepSeek-style fusion (enorm + hnorm +
//! eh_proj + mtp_block + shared_head) with PanguDecoderLayer transformer blocks.
//!
//! # Architecture
//!
//! ```text
...
//!   - `model.layers.{mtp_start+i}.mtp_block.*`  (PanguDecoderLayer structure)
//!
//! # Reference
//!
//! `reference/vllm/vllm/model_executor/models/openpangu_mtp.py`
...
use super::pangu::PanguDecoderLayer;

// ─── Layer ──────────────────────────────────────────────────────────────────

/// One OpenPangu MTP prediction layer.
struct OpenPanguMtpLayer {
    enorm: RmsNorm,
    hnorm: RmsNorm,
    eh_proj: Linear,
    shared_head_norm: RmsNorm,
    shared_head: Linear,
    mtp_block: PanguDecoderLayer,
}

impl OpenPanguMtpLayer {
    fn new(cfg: &ModelConfig, abs_layer_idx: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let mtp_block = PanguDecoderLayer::new_for_mtp(cfg, abs_layer_idx, vb.pp("mtp_block"))?;
        Ok(Self {
            enorm,
            hnorm,
            eh_proj,
...
/// OpenPangu Multi-Token Prediction draft model.
///
/// Supports multiple MTP layers; `spec_step_idx` cycles through them.
pub struct OpenPanguMtpModel {
    embed_tokens: Embedding,
    layers: Vec<OpenPanguMtpLayer>,
    tp_ctx: TpContext,
    device: Device,
}

impl OpenPanguMtpModel {
    /// Load the OpenPangu MTP model from a VarBuilder rooted at the model file.
    ///
    ...
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..num_mtp {
            ...
            layers.push(OpenPanguMtpLayer::new(cfg, abs_idx, vb_l.pp(abs_idx))?);
        }
...
impl MtpDraftModel for OpenPanguMtpModel {
    fn forward(
        &self,
        input_ids: &Tensor,
...
mod tests {
    ...
    fn make_cfg() -> ModelConfig {
        ...
        // All layers dense: first_k_dense_replace > num_hidden_layers
        extra.insert("first_k_dense_replace".to_string(), json!(10));

        ModelConfig {
            architectures: vec!["OpenPanguMTPModel".to_string()],
            hidden_size: 64,
    ...
    #[test]
    fn test_openpangu_mtp_new() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = OpenPanguMtpModel::new(&cfg, vb);
        assert!(
            model.is_ok(),
            "OpenPanguMtpModel creation failed: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_openpangu_mtp_prefill_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = OpenPanguMtpModel::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_openpangu_mtp_decode_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = OpenPanguMtpModel::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_openpangu_mtp_logits_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = OpenPanguMtpModel::new(&cfg, vb).unwrap();
