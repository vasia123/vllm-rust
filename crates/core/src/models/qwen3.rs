...
impl Qwen3Attention {
    fn new_with_tp(cfg: &ModelConfig, vb: VarBuilder, pg: &dyn ProcessGroup) -> Result<Self> {
        ...
        // Qwen3-specific: per-head RMSNorm on Q and K (not TP, operates on head_dim)
    ...
    fn forward_decode_batch(
        ...
        // Batched per-head RMSNorm (operates element-wise, safe across batch)
        let q = apply_per_head_norm(&q, &self.q_norm)?;
        let k = apply_per_head_norm(&k, &self.k_norm)?;
...
impl Qwen3DecoderLayer {
    fn new_with_tp(cfg: &ModelConfig, vb: VarBuilder, pg: &dyn ProcessGroup) -> Result<Self> {
        let self_attn = Qwen3Attention::new_with_tp(cfg, vb.pp("self_attn"), pg)?;
...
impl Qwen3ForCausalLM {
    /// Create a new Qwen3 model for single GPU.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        Self::new_with_tp(cfg, vb, &LocalProcessGroup::new(), TpContext::single_gpu())
    }

    /// Create a new Qwen3 model with tensor parallelism.
    ///
    /// # Arguments
    ...
    pub fn new_with_tp(
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(Qwen3DecoderLayer::new_with_tp(cfg, vb_l.pp(i), pg)?);
        }
    ...
    /// Embed token IDs → hidden states [B, S, hidden] for VLM use.
    pub(crate) fn embed_text(&self, input_ids: &Tensor) -> Result<Tensor> {
        self.embed_tokens.forward(input_ids, &self.tp_ctx)
    }

    /// Run LLM layers + norm + lm_head on pre-embedded tensor for VLM use.
    ...
    /// Batch decode path with pre-embedded tensor for VLM use.
...
mod tests {
    ...
    fn test_qwen3_construction() {
        ...
        assert!(
            model.is_ok(),
            "Qwen3ForCausalLM should construct with zero weights"
        );
    ...
    #[test]
    fn test_qwen3_forward_shape_no_download() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Qwen3ForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_qwen3_single_token_forward() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Qwen3ForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_qwen3_has_per_head_norm() {
        // Qwen3 architecture uses per-head RMSNorm on Q and K
        // This is a key difference from Llama
        ...
        // Qwen3Attention should create q_norm and k_norm via VarBuilder
        let attn = Qwen3Attention::new_with_tp(&cfg, vb.pp("self_attn"), &pg);
        assert!(
            attn.is_ok(),
            "Qwen3Attention should construct with q_norm and k_norm"
        );
    ...
    #[test]
    fn test_qwen3_attention_layer_structure() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let pg = LocalProcessGroup::new();

        let layer = Qwen3DecoderLayer::new_with_tp(&cfg, vb.pp("layer"), &pg);
        assert!(layer.is_ok(), "Qwen3DecoderLayer should construct");

        // Verify the layer has all expected components
        // (input_layernorm, self_attn with q_norm/k_norm, post_attention_layernorm, mlp)
    }

    #[test]
    fn test_qwen3_gqa_configuration() {
        let cfg = test_config();
    ...
    #[test]
    fn test_qwen3_prefill_then_decode() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Qwen3ForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_qwen3_untied_embeddings() {
    ...
    #[test]
    fn test_qwen3_tp_construction_world_size_2() {
        ...
        let model = Qwen3ForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx);
        assert!(
            model.is_ok(),
            "Qwen3ForCausalLM should construct with TP=2: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_qwen3_tp_forward_world_size_2() {
        let cfg = test_config_tp_compatible();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let model = Qwen3ForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx).expect("build model");
    ...
    #[test]
    fn test_qwen3_tp_heads_divisibility_check() {
        ...
        let result = Qwen3ForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx);
