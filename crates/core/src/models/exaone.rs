...
// Exaone = vanilla GQA. Output proj is `out_proj`, projections nested under
// `attention` sub-VarBuilder.
...
impl ExaoneAttention {
    fn new_with_tp(cfg: &ModelConfig, vb: VarBuilder, pg: &dyn ProcessGroup) -> Result<Self> {
        ...
        // Exaone nests projections under "attention".
        let inner = AttentionBlock::new(&attn_cfg, vb.pp("attention"), pg, rotary_emb)?;
        Ok(Self { inner })
    }
