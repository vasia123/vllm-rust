//! Mixtral model with LoRA adapter support.
//!
//! Mixtral is a Mixture of Experts (MoE) model based on Mistral architecture.
//! LoRA is applied only to the attention projections (q, k, v, o) since applying
//! LoRA to individual MoE experts would be prohibitively expensive (8 experts x
//! 3 projections = 24 adapters per layer). The MoE routing and expert weights
//! are left unchanged.
...
use super::mixtral::MixtralConfig;

// ─── Attention with LoRA ─────────────────────────────────────────────────────

struct MixtralAttentionWithLora {
...
struct MixtralDecoderLayerWithLora {
    self_attn: MixtralAttentionWithLora,
    block_sparse_moe: MoELayer,
    input_layernorm: RmsNorm,
    post_attention_layernorm: RmsNorm,
}

impl MixtralDecoderLayerWithLora {
    fn new(cfg: &MixtralConfig, vb: VarBuilder) -> Result<Self> {
        let self_attn = MixtralAttentionWithLora::new(&cfg.base, vb.pp("self_attn"))?;
    ...
    fn register_lora(&mut self, adapter_name: &str, lora_model: &LoraModel, layer_idx: usize) {
        ...
        // MoE experts are not LoRA-adapted: too many parameters (num_experts * 3 projections)
    }
...
/// Mixtral MoE model with LoRA adapter support.
///
/// LoRA adapters are applied only to attention projections (q, k, v, o).
/// MoE expert weights and routing are left unchanged since applying LoRA
/// to individual experts would be prohibitively expensive.
pub struct MixtralWithLora {
    embed_tokens: Embedding,
    layers: Vec<MixtralDecoderLayerWithLora>,
    norm: RmsNorm,
...
impl MixtralWithLora {
    /// Create a new Mixtral model with LoRA support.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(MixtralDecoderLayerWithLora::new(&mixtral_cfg, vb_l.pp(i))?);
        }
...
impl crate::engine::ModelForward for MixtralWithLora {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
impl crate::models::ModelForwardWithLora for MixtralWithLora {
    fn register_lora(&mut self, lora_model: &LoraModel) {
        MixtralWithLora::register_lora(self, lora_model)
    }
...
mod tests {
    ...
    fn test_config() -> crate::config::ModelConfig {
        ...
        extra.insert("num_local_experts".to_string(), serde_json::json!(8));
    ...
    #[test]
    fn test_mixtral_with_lora_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = MixtralWithLora::new(&cfg, vb);
        assert!(
            model.is_ok(),
            "MixtralWithLora should construct: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_mixtral_with_lora_forward_no_adapter() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = MixtralWithLora::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_mixtral_with_lora_register_and_forward() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let mut model = MixtralWithLora::new(&cfg, vb).unwrap();
