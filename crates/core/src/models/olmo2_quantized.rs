//! Quantized OLMo2 model implementation.
//!
//! This module provides a quantized version of the OLMo2 model that supports
...
//! - Post-normalization: norms applied after residual add (not before sublayer)
...
// ─── Quantized Attention ─────────────────────────────────────────────────────
//
// OLMo2-specific: QK normalization via per-head RMSNorm before RoPE.

struct QuantizedOlmo2Attention {
...
impl QuantizedOlmo2Attention {
    fn new(
        ...
        // OLMo2-specific: per-head QK normalization (loaded via VarBuilder, not quantized)
...
//
// OLMo2 post-normalization: norms applied AFTER residual add.

struct QuantizedOlmo2DecoderLayer {
    self_attn: QuantizedOlmo2Attention,
    mlp: QuantizedSwiGluMlp,
    post_attention_layernorm: RmsNorm,
    post_feedforward_layernorm: RmsNorm,
}

impl QuantizedOlmo2DecoderLayer {
    fn new(
        cfg: &ModelConfig,
        loader: &dyn QuantizedWeightLoader,
        vb: VarBuilder,
        ...
        let self_attn = QuantizedOlmo2Attention::new(
            cfg,
            loader,
            vb_layer.clone(),
    ...
    fn forward(
        ...
        // Post-norm: attention on raw input, then norm after residual add
        let residual = xs;
...
/// Quantized OLMo2 model supporting FP8, GPTQ, AWQ, and unquantized weights.
pub struct QuantizedOlmo2ForCausalLM {
    embed_tokens: Embedding,
    layers: Vec<QuantizedOlmo2DecoderLayer>,
    norm: RmsNorm,
...
impl QuantizedOlmo2ForCausalLM {
    pub fn new(
        cfg: &ModelConfig,
        vb: VarBuilder,
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(QuantizedOlmo2DecoderLayer::new(
                cfg,
                weight_loader,
                vb.clone(),
                i,
...
impl crate::engine::ModelForward for QuantizedOlmo2ForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    #[test]
    fn test_quantized_olmo2_construction() {
        let cfg = test_config();
        ...
        let model = QuantizedOlmo2ForCausalLM::new(&cfg, vb, loader.as_ref());
        assert!(
            model.is_ok(),
            "QuantizedOlmo2ForCausalLM should construct: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_quantized_olmo2_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let detected = DetectedQuantConfig::default();
        let loader = create_weight_loader_with_params(vb.clone(), &detected);
        let model = QuantizedOlmo2ForCausalLM::new(&cfg, vb, loader.as_ref()).expect("build model");
    ...
    #[test]
    fn test_quantized_olmo2_with_gptq_loader() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
    ...
    #[test]
    fn test_quantized_olmo2_untied_embeddings() {
        ...
        assert!(
            model.is_ok(),
            "Untied embeddings should construct: {:?}",
            model.err()
        );
    }
}
