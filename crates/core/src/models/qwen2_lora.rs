//! Qwen2 model with LoRA adapter support.
//!
//! Qwen2 differs from Llama/Qwen3 in using attention bias on QKV projections.
//! LoRA is applied to: q_proj, k_proj, v_proj (with bias), o_proj (no bias),
...
impl Qwen2AttentionWithLora {
    fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        let num_heads = cfg.num_attention_heads;
        let num_kv_heads = cfg.num_key_value_heads;
        let head_dim = cfg.head_dim;

        // Qwen2 uses bias=True for QKV projections
        let use_bias = cfg.attention_bias.unwrap_or(true);

        let q_proj = if use_bias {
            let base = candle_nn::linear(cfg.hidden_size, num_heads * head_dim, vb.pp("q_proj"))?;
            LinearWithLora::from_linear(base)
        } else {
            LinearWithLora::new(cfg.hidden_size, num_heads * head_dim, vb.pp("q_proj"))?
        };
...
struct Qwen2DecoderLayerWithLora {
    self_attn: Qwen2AttentionWithLora,
    mlp: SwiGluMlpWithLora,
    input_layernorm: RmsNorm,
    post_attention_layernorm: RmsNorm,
}

impl Qwen2DecoderLayerWithLora {
    fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        let self_attn = Qwen2AttentionWithLora::new(cfg, vb.pp("self_attn"))?;
...
/// Qwen2 model with LoRA adapter support.
///
/// Key difference from Qwen3WithLora: uses attention bias on QKV projections
/// and does NOT have per-head QK normalization.
pub struct Qwen2WithLora {
    embed_tokens: Embedding,
    layers: Vec<Qwen2DecoderLayerWithLora>,
    norm: RmsNorm,
...
impl Qwen2WithLora {
    /// Create a new Qwen2 model with LoRA support.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(Qwen2DecoderLayerWithLora::new(cfg, vb_l.pp(i))?);
        }
...
impl crate::engine::ModelForward for Qwen2WithLora {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
impl crate::models::ModelForwardWithLora for Qwen2WithLora {
    fn register_lora(&mut self, lora_model: &LoraModel) {
        Qwen2WithLora::register_lora(self, lora_model)
    }
...
mod tests {
    ...
    #[test]
    fn test_qwen2_with_lora_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = Qwen2WithLora::new(&cfg, vb);
        assert!(model.is_ok(), "Qwen2WithLora should construct");
        assert_eq!(model.unwrap().layers.len(), cfg.num_hidden_layers);
    }

    #[test]
    fn test_qwen2_with_lora_construction_no_bias() {
        ...
        assert!(
            model.is_ok(),
            "Qwen2WithLora should construct without attention bias"
        );
    }

    #[test]
    fn test_qwen2_with_lora_forward_no_adapter() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Qwen2WithLora::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_qwen2_with_lora_register_and_forward() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let mut model = Qwen2WithLora::new(&cfg, vb).unwrap();
