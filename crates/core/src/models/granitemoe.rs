...
// GraniteMoE = vanilla GQA (no bias) + custom attention scaling. Applies
// attention_multiplier consistently across all paths (fixes the latent
// prefill-vs-decode inconsistency in the bespoke implementation).
...
impl GraniteMoeAttention {
    fn new_with_tp(
        ...
        if (gmoe_cfg.attention_multiplier - default_scale).abs() > f64::EPSILON {
            attn_cfg = attn_cfg.with_scale(gmoe_cfg.attention_multiplier);
        }
