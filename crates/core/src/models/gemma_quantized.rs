//! Quantized Gemma model implementation.
//!
//! This module provides a quantized version of the Gemma model that supports
...
struct QuantizedGemmaAttention {
...
struct QuantizedGemmaDecoderLayer {
    self_attn: QuantizedGemmaAttention,
    mlp: QuantizedGeGluMlp,
    input_layernorm: GemmaRmsNorm,
    post_attention_layernorm: GemmaRmsNorm,
}

impl QuantizedGemmaDecoderLayer {
    fn new(
        cfg: &ModelConfig,
        loader: &dyn QuantizedWeightLoader,
        vb: VarBuilder,
        ...
        let self_attn = QuantizedGemmaAttention::new(cfg, loader, &format!("{prefix}.self_attn"))?;
...
/// Quantized Gemma model supporting FP8, GPTQ, AWQ, and unquantized weights.
pub struct QuantizedGemmaForCausalLM {
    embed_tokens: Embedding,
    layers: Vec<QuantizedGemmaDecoderLayer>,
    norm: GemmaRmsNorm,
...
impl QuantizedGemmaForCausalLM {
    pub fn new(
        cfg: &ModelConfig,
        vb: VarBuilder,
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(QuantizedGemmaDecoderLayer::new(
                cfg,
                weight_loader,
                vb.clone(),
                i,
...
impl crate::engine::ModelForward for QuantizedGemmaForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    #[test]
    fn test_quantized_gemma_construction() {
        let cfg = test_config();
        ...
        let model = QuantizedGemmaForCausalLM::new(&cfg, vb, loader.as_ref());
        assert!(
            model.is_ok(),
            "QuantizedGemmaForCausalLM should construct with unquantized loader"
        );
    ...
    #[test]
    fn test_quantized_gemma_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let detected = DetectedQuantConfig::default();
        let loader = create_weight_loader_with_params(vb.clone(), &detected);
        let model = QuantizedGemmaForCausalLM::new(&cfg, vb, loader.as_ref()).expect("build model");
