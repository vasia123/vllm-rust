//! PhiMoE model implementation (Microsoft Phi with Mixture of Experts).
//!
//! Key differences from standard Llama MoE models:
//! - LayerNorm instead of RMSNorm
//! - Attention projections use bias (q, k, v, o all have bias)
//! - lm_head uses bias
//! - MoE routing uses top-2 without renormalization (sparsemixer at inference)
//! - Default 16 experts, top-2 routing
//! - Default rope_theta = 1e6
//!
//! Reference: vLLM phimoe.py
...
// ─── PhiMoE config extraction ────────────────────────────────────────────────

struct PhiMoeExtraConfig {
    num_local_experts: usize,
    num_experts_per_tok: usize,
}

impl PhiMoeExtraConfig {
...
// PhiMoE = vanilla GQA + bias on ALL four projections (Q/K/V/O).
...
impl PhiMoeDecoderLayer {
    fn new_with_tp(
        cfg: &ModelConfig,
        moe_cfg: &PhiMoeExtraConfig,
        vb: VarBuilder,
        pg: &dyn ProcessGroup,
    ) -> Result<Self> {
        let self_attn = PhiMoeAttention::new_with_tp(cfg, vb.pp("self_attn"), pg)?;
        ...
        // PhiMoE uses LayerNorm (not RMSNorm)
        let input_layernorm =
...
pub struct PhiMoeForCausalLM {
    embed_tokens: TpEmbedding,
    layers: Vec<PhiMoeDecoderLayer>,
    norm: LayerNorm,
    lm_head: TpLinear,
    tp_ctx: TpContext,
    device: Device,
...
impl PhiMoeForCausalLM {
    ...
    pub fn new_with_tp(
        ...
        let moe_cfg = PhiMoeExtraConfig::from_model_config(cfg);
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(PhiMoeDecoderLayer::new_with_tp(
                cfg,
                &moe_cfg,
                vb_l.pp(i),
                pg,
        ...
        // PhiMoE uses LayerNorm (not RMSNorm) for final norm
        let norm = layer_norm(cfg.hidden_size, cfg.rms_norm_eps, vb_m.pp("norm"))?;

        // PhiMoE lm_head has bias
...
impl crate::engine::ModelForward for PhiMoeForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    fn test_config() -> ModelConfig {
        ...
        ModelConfig {
            architectures: vec!["PhiMoEForCausalLM".to_string()],
            hidden_size: 64,
    ...
    #[test]
    fn test_phimoe_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = PhiMoeForCausalLM::new(&cfg, vb);
        assert!(
            model.is_ok(),
            "PhiMoeForCausalLM should construct: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_phimoe_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = PhiMoeForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_phimoe_default_experts() {
        // Verify defaults when MoE config is absent
        let cfg = ModelConfig {
            architectures: vec!["PhiMoEForCausalLM".to_string()],
            hidden_size: 64,
            num_attention_heads: 4,
        ...
        let moe_cfg = PhiMoeExtraConfig::from_model_config(&cfg);
        assert_eq!(moe_cfg.num_local_experts, 16);
        assert_eq!(moe_cfg.num_experts_per_tok, 2);
    }

    #[test]
    fn test_phimoe_prefill_then_decode() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = PhiMoeForCausalLM::new(&cfg, vb).expect("build model");
