...
// Eagle3 = vanilla GQA + RoPE + optional QKV bias (o_proj never biased).
// Layer 0 has `qkv_input_size = 2 * hidden_size` (concat of token embeds
// and target hidden states); subsequent layers use `hidden_size`. The
...
impl Eagle3Attention {
    fn new(
        ...
        .with_bias(bias);
        if qkv_input_size != cfg.hidden_size {
            attn_cfg = attn_cfg.with_qkv_input_size(qkv_input_size);
        }
