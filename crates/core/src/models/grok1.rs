...
// ─── Attention ──────────────────────────────────────────────────────────────
//
// Grok1 = vanilla GQA + RoPE + post-attention output multiplier (model-
// specific scalar applied after o_proj). The multiplier is applied
// externally — it commutes with the linear o_proj inside AttentionBlock,
// so post-multiplying the final tensor is bit-equivalent to the original
// implementation.
