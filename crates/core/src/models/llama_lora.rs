//! Llama model with LoRA adapter support.
...
impl LlamaAttentionWithLora {
    ...
    /// Register LoRA adapters for this attention layer.
...
struct LlamaDecoderLayerWithLora {
    self_attn: LlamaAttentionWithLora,
    mlp: SwiGluMlpWithLora,
    input_layernorm: RmsNorm,
    post_attention_layernorm: RmsNorm,
}

impl LlamaDecoderLayerWithLora {
    fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        let self_attn = LlamaAttentionWithLora::new(cfg, vb.pp("self_attn"))?;
...
/// Llama model with LoRA adapter support.
///
/// This is a variant of LlamaForCausalLM that uses LinearWithLora layers,
/// enabling per-request adapter selection for fine-tuned behavior.
pub struct LlamaWithLora {
    embed_tokens: Embedding,
    layers: Vec<LlamaDecoderLayerWithLora>,
    norm: RmsNorm,
...
impl LlamaWithLora {
    /// Create a new Llama model with LoRA support.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(LlamaDecoderLayerWithLora::new(cfg, vb_l.pp(i))?);
        }
...
impl crate::engine::ModelForward for LlamaWithLora {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
impl crate::models::ModelForwardWithLora for LlamaWithLora {
    fn register_lora(&mut self, lora_model: &LoraModel) {
        LlamaWithLora::register_lora(self, lora_model)
    }
    ...
    fn lora_adapters(&self) -> Vec<String> {
        // Collect adapter names from the first layer's first LoRA-enabled projection
...
mod tests {
    ...
    #[test]
    fn test_llama_with_lora_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = LlamaWithLora::new(&cfg, vb);
        assert!(model.is_ok(), "LlamaWithLora should construct");
        assert_eq!(model.unwrap().layers.len(), cfg.num_hidden_layers);
    }

    #[test]
    fn test_llama_with_lora_forward_no_adapter() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = LlamaWithLora::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_llama_with_lora_register_and_forward() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let mut model = LlamaWithLora::new(&cfg, vb).unwrap();

        // Create a mock LoRA model
        ...
        // Verify adapter is registered on all layers
        ...
        // Verify forward passes work with LoRA context (even if output is same due to zero embeddings)
        let cache_config = create_cache_config(&cfg, &device);
        let mut kv_cache_mgr = KVCacheManager::new(&cache_config).unwrap();
        let mut block_table = crate::kv_cache::BlockTable::new(cache_config.block_size);
