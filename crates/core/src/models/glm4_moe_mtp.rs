//! GLM4-MoE Multi-Token Prediction (MTP) draft model.
//!
//! GLM4-MoE MTP uses the standard DeepSeek-style fusion (enorm + hnorm +
//! eh_proj + mtp_block + shared_head) with GLM4-MoE decoder blocks. Multiple
//! MTP layers are supported; `spec_step_idx` cycles through them.
//!
//! This implementation covers both `Glm4MoeMTPModel` and `Glm4MoeLiteMTPModel`
//! since they share the same architecture on the Rust side.
//!
//! # Architecture
//!
//! ```text
...
//!   - `model.layers.{mtp_start+i}.mtp_block.*`  (Glm4MoeDecoderLayer structure)
//!
//! # Reference
//!
//! `reference/vllm/vllm/model_executor/models/glm4_moe_mtp.py`
...
use super::glm4_moe::Glm4MoeDecoderLayer;
use super::mtp_base::MtpDraftModel;

// ─── Layer ──────────────────────────────────────────────────────────────────

/// One GLM4-MoE MTP prediction layer.
struct Glm4MoeMtpLayer {
    enorm: RmsNorm,
    hnorm: RmsNorm,
    eh_proj: Linear,
...
impl Glm4MoeMtpLayer {
    fn new(cfg: &ModelConfig, abs_layer_idx: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let mtp_block = Glm4MoeDecoderLayer::new_for_mtp(cfg, abs_layer_idx, vb.pp("mtp_block"))?;
        Ok(Self {
            enorm,
            hnorm,
            eh_proj,
...
/// GLM4-MoE Multi-Token Prediction draft model.
///
/// Supports multiple MTP layers; `spec_step_idx` is used to cycle through them.
/// Also registered under `Glm4MoeLiteMTPModel`.
pub struct Glm4MoeMtpModel {
    embed_tokens: Embedding,
    layers: Vec<Glm4MoeMtpLayer>,
    tp_ctx: TpContext,
    device: Device,
}

impl Glm4MoeMtpModel {
    /// Load the GLM4-MoE MTP model from a VarBuilder rooted at the model file.
    ///
    /// Layer count is read from `num_nextn_predict_layers` in `cfg.extra` (defaults to 1).
    /// MTP layers start at absolute index `num_hidden_layers`.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..num_mtp {
            ...
            layers.push(Glm4MoeMtpLayer::new(cfg, abs_idx, vb_l.pp(abs_idx))?);
        }
...
impl MtpDraftModel for Glm4MoeMtpModel {
    fn forward(
        &self,
        input_ids: &Tensor,
        ...
        // BOS masking: zero the first position embedding during prefill
...
mod tests {
    ...
    fn make_cfg() -> ModelConfig {
        ...
        // num_nextn_predict_layers controls the number of MTP layers
        extra.insert("num_nextn_predict_layers".to_string(), json!(2));
        // Set first_k_dense_replace > num_hidden_layers so all target layers are dense.
        // The MTP layer itself is at abs_idx >= num_hidden_layers, so it's also dense.
        extra.insert("first_k_dense_replace".to_string(), json!(10));
        // use_qk_norm must be present for Glm4MoeAttention
        extra.insert("use_qk_norm".to_string(), json!(false));

        ModelConfig {
            architectures: vec!["Glm4MoeMTPModel".to_string()],
            hidden_size: 64,
    ...
    #[test]
    fn test_glm4_moe_mtp_new() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = Glm4MoeMtpModel::new(&cfg, vb);
        assert!(
            model.is_ok(),
            "Glm4MoeMtpModel creation failed: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_glm4_moe_mtp_prefill_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = Glm4MoeMtpModel::new(&cfg, vb).unwrap();
        let num_mtp = model.num_mtp_layers();
        let mut kv_cache = make_cache(&cfg, num_mtp, &device);
    ...
    #[test]
    fn test_glm4_moe_mtp_decode_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = Glm4MoeMtpModel::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_glm4_moe_mtp_logits_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = Glm4MoeMtpModel::new(&cfg, vb).unwrap();
