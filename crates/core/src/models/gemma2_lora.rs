//! Gemma2 model with LoRA adapter support.
//!
//! Gemma2 extends Gemma with attention logit soft-capping, final logit
//! soft-capping, alternating sliding window attention, a custom query
//! pre-attention scalar, and four layernorms per decoder layer.
//! LoRA is applied to all attention projections (q, k, v, o)
//! and MLP projections (gate_proj, up_proj, down_proj).
...
struct Gemma2AttentionWithLora {
...
impl Gemma2AttentionWithLora {
    ...
    /// Prefill forward: computes full attention manually with soft-capping and
    /// optional sliding window mask, then writes KV to cache.
    ...
    /// Decode forward: processes each sequence individually to apply
    /// soft-capping and optional sliding window correctly.
...
struct Gemma2DecoderLayerWithLora {
    self_attn: Gemma2AttentionWithLora,
...
impl Gemma2DecoderLayerWithLora {
    fn new(cfg: &ModelConfig, layer_idx: usize, vb: VarBuilder) -> Result<Self> {
...
/// Gemma2 model with LoRA adapter support.
///
/// Extends the Gemma architecture with attention logit soft-capping, final logit
/// soft-capping, alternating sliding window attention, custom query scaling,
/// and four layernorms per decoder layer. Embeddings are scaled by sqrt(hidden_size).
pub struct Gemma2WithLora {
    embed_tokens: Embedding,
    layers: Vec<Gemma2DecoderLayerWithLora>,
    norm: Gemma2RmsNorm,
...
impl Gemma2WithLora {
    /// Create a new Gemma2 model with LoRA support.
...
impl crate::engine::ModelForward for Gemma2WithLora {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
impl crate::models::ModelForwardWithLora for Gemma2WithLora {
    fn register_lora(&mut self, lora_model: &LoraModel) {
        Gemma2WithLora::register_lora(self, lora_model)
    }
...
mod tests {
    ...
    #[test]
    fn test_gemma2_with_lora_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = Gemma2WithLora::new(&cfg, vb);
        assert!(model.is_ok(), "Gemma2WithLora should construct");
    ...
    #[test]
    fn test_gemma2_with_lora_forward_no_adapter() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Gemma2WithLora::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_gemma2_with_lora_register_and_forward() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let mut model = Gemma2WithLora::new(&cfg, vb).unwrap();
