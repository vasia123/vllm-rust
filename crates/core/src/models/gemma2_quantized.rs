//! Quantized Gemma2 model implementation.
//!
//! This module provides a quantized version of the Gemma2 model that supports
//! various quantization methods (FP8, GPTQ, AWQ) through the QuantizedWeightLoader
//! abstraction.
//!
//! Gemma2-specific features preserved in the quantized path:
...
struct QuantizedGemma2Attention {
...
struct QuantizedGemma2DecoderLayer {
    self_attn: QuantizedGemma2Attention,
...
impl QuantizedGemma2DecoderLayer {
    fn new(
        cfg: &ModelConfig,
        loader: &dyn QuantizedWeightLoader,
        vb: VarBuilder,
        ...
        let self_attn =
            QuantizedGemma2Attention::new(cfg, loader, layer_idx, &format!("{prefix}.self_attn"))?;
        ...
        let input_layernorm = Gemma2RmsNorm::new(
            cfg.hidden_size,
            cfg.rms_norm_eps,
...
/// Quantized Gemma2 model supporting FP8, GPTQ, AWQ, and unquantized weights.
pub struct QuantizedGemma2ForCausalLM {
    embed_tokens: Embedding,
    layers: Vec<QuantizedGemma2DecoderLayer>,
    norm: Gemma2RmsNorm,
...
impl QuantizedGemma2ForCausalLM {
    pub fn new(
        cfg: &ModelConfig,
        vb: VarBuilder,
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(QuantizedGemma2DecoderLayer::new(
                cfg,
                weight_loader,
                vb.clone(),
                i,
...
impl crate::engine::ModelForward for QuantizedGemma2ForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    #[test]
    fn test_quantized_gemma2_construction() {
        let cfg = test_config();
        ...
        let model = QuantizedGemma2ForCausalLM::new(&cfg, vb, loader.as_ref());
        assert!(
            model.is_ok(),
            "QuantizedGemma2ForCausalLM should construct with unquantized loader"
        );
    ...
    #[test]
    fn test_quantized_gemma2_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let loader = create_weight_loader_with_params(vb.clone(), &detected);
        let model =
            QuantizedGemma2ForCausalLM::new(&cfg, vb, loader.as_ref()).expect("build model");
