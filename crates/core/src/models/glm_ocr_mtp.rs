//! GLM-OCR Multi-Token Prediction (MTP) draft model.
//!
//! GLM-OCR MTP uses the standard DeepSeek-style fusion (enorm + hnorm +
//! eh_proj + mtp_block + shared_head) with the dense Glm4DecoderLayer blocks.
//! The model is a VLM wrapper: its config nests the language model config under
//! `text_config`.
//!
//! # Architecture
//!
...
//!   - `model.layers.{mtp_start+i}.mtp_block.*`  (Glm4DecoderLayer structure)
//!
//! # Reference
//!
//! `reference/vllm/vllm/model_executor/models/glm_ocr_mtp.py`
...
use super::glm4::Glm4DecoderLayer;
use super::mtp_base::MtpDraftModel;

// ─── Layer ──────────────────────────────────────────────────────────────────

/// One GLM-OCR MTP prediction layer.
struct GlmOcrMtpLayer {
    enorm: RmsNorm,
    hnorm: RmsNorm,
    eh_proj: Linear,
...
impl GlmOcrMtpLayer {
    fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let mtp_block = Glm4DecoderLayer::new_for_mtp(cfg, vb.pp("mtp_block"))?;
        Ok(Self {
            enorm,
            hnorm,
            eh_proj,
...
/// GLM-OCR Multi-Token Prediction draft model.
///
/// Supports multiple MTP layers; `spec_step_idx` cycles through them.
pub struct GlmOcrMtpModel {
    embed_tokens: Embedding,
    layers: Vec<GlmOcrMtpLayer>,
    tp_ctx: TpContext,
    device: Device,
}

impl GlmOcrMtpModel {
    /// Load the GLM-OCR MTP model from a VarBuilder rooted at the model file.
    ///
    /// Layer count from `num_nextn_predict_layers` in `cfg.extra` (defaults to 1).
    /// GLM-OCR is a VLM: its language config may be nested under `text_config`.
    /// The `cfg` passed here should already be the language model config or
    /// have `hidden_size` accessible directly.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..num_mtp {
            ...
            layers.push(GlmOcrMtpLayer::new(cfg, vb_l.pp(abs_idx))?);
        }
...
impl MtpDraftModel for GlmOcrMtpModel {
    fn forward(
        &self,
        input_ids: &Tensor,
...
mod tests {
    ...
    fn make_cfg() -> ModelConfig {
        ...
        ModelConfig {
            architectures: vec!["GlmOcrMTPModel".to_string()],
            hidden_size: 64,
    ...
    #[test]
    fn test_glm_ocr_mtp_new() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = GlmOcrMtpModel::new(&cfg, vb);
        assert!(
            model.is_ok(),
            "GlmOcrMtpModel creation failed: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_glm_ocr_mtp_prefill_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = GlmOcrMtpModel::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_glm_ocr_mtp_decode_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = GlmOcrMtpModel::new(&cfg, vb).unwrap();
    ...
    #[test]
    fn test_glm_ocr_mtp_logits_shape() {
        let device = Device::Cpu;
        let cfg = make_cfg();
        let vb = VarBuilder::zeros(DType::F32, &device);
        let model = GlmOcrMtpModel::new(&cfg, vb).unwrap();
