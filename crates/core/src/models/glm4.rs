...
//!
//! Glm4ForCausalLM has a unique architecture with 4 layer norms per decoder:
...
//! - is_neox_style = false
//!
//! Reference: vLLM's glm4.py implementation.
...
// GLM4 = vanilla GQA + bias on Q/K/V (configurable, default false) + partial
// RoPE (rotary_dim = head_dim/2, split-style).
...
/// GLM4 decoder layer with 4 layer norms.
///
...
impl Glm4DecoderLayer {
    fn new_with_tp(cfg: &ModelConfig, vb: VarBuilder, pg: &dyn ProcessGroup) -> Result<Self> {
        let self_attn = Glm4Attention::new_with_tp(cfg, vb.pp("self_attn"), pg)?;
    ...
    pub(crate) fn forward(
        ...
        // Post self-attention norm (GLM4-specific)
...
///
/// GLM-4 has a unique decoder architecture with 4 layer norms per layer.
...
impl Glm4ForCausalLM {
    /// Create a new GLM4 model for single GPU.
    pub fn new(cfg: &ModelConfig, vb: VarBuilder) -> Result<Self> {
        Self::new_with_tp(cfg, vb, &LocalProcessGroup::new(), TpContext::single_gpu())
    }

    /// Create a new GLM4 model with tensor parallelism.
    ///
    /// # Arguments
    ...
    pub fn new_with_tp(
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(Glm4DecoderLayer::new_with_tp(cfg, vb_l.pp(i), pg)?);
        }
...
impl crate::engine::ModelForward for Glm4ForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    #[test]
    fn test_glm4_construction() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);

        let model = Glm4ForCausalLM::new(&cfg, vb);
        assert!(model.is_ok(), "Glm4ForCausalLM should construct");
    ...
    #[test]
    fn test_glm4_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Glm4ForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_glm4_decoder_layer_structure() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let pg = LocalProcessGroup::new();

        // Should create successfully with 4 layer norms
        let layer = Glm4DecoderLayer::new_with_tp(&cfg, vb.pp("layer"), &pg);
        assert!(
            layer.is_ok(),
            "Glm4DecoderLayer should construct with 4 layer norms"
        );
    }

    #[test]
    fn test_glm4_prefill_then_decode() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        let model = Glm4ForCausalLM::new(&cfg, vb).expect("build model");
    ...
    #[test]
    fn test_glm4_untied_embeddings() {
    ...
    #[test]
    fn test_glm4_tp_construction_world_size_2() {
        ...
        let model = Glm4ForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx);
        assert!(
            model.is_ok(),
            "Glm4ForCausalLM should construct with TP=2: {:?}",
            model.err()
        );
    ...
    #[test]
    fn test_glm4_tp_forward_world_size_2() {
        let cfg = test_config_tp_compatible();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let model = Glm4ForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx).expect("build model");
    ...
    #[test]
    fn test_glm4_tp_heads_divisibility_check() {
        ...
        let result = Glm4ForCausalLM::new_with_tp(&cfg, vb, &pg, tp_ctx);
