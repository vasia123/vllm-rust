...
// Qwen3-MoE = vanilla GQA + per-head QK RMSNorm. Same shape as Qwen3.
...
impl Qwen3MoeForCausalLM {
    ...
    /// Run transformer layers + norm + lm_head on pre-embedded tensor.
    ...
    /// Batch decode with pre-embedded tensor.
...
mod tests {
    ...
    fn test_qwen3_moe_tp_attention_local_heads() {
        ...
        assert_eq!(attn.inner.num_heads(), 2);
        assert_eq!(attn.inner.num_kv_heads(), 1);
    }
