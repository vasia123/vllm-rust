//! Quantized Qwen2 model implementation.
//!
//! This module provides a quantized version of the Qwen2 model that supports
//! various quantization methods (FP8, GPTQ, AWQ) through the QuantizedWeightLoader
//! abstraction.
//!
//! Key difference from Llama/Qwen3: Qwen2 uses bias on QKV projections.
...
impl QuantizedQwen2Attention {
    fn new(cfg: &ModelConfig, loader: &dyn QuantizedWeightLoader, prefix: &str) -> Result<Self> {
        ...
        // Qwen2 uses bias=True for QKV projections by default
...
impl QuantizedQwen2DecoderLayer {
    fn new(
        ...
        let self_attn = QuantizedQwen2Attention::new(cfg, loader, &format!("{prefix}.self_attn"))?;
...
/// Quantized Qwen2 model supporting FP8, GPTQ, AWQ, and unquantized weights.
...
impl QuantizedQwen2ForCausalLM {
    pub fn new(
        ...
        for i in 0..cfg.num_hidden_layers {
            layers.push(QuantizedQwen2DecoderLayer::new(
                cfg,
                weight_loader,
                vb.clone(),
                i,
...
impl crate::engine::ModelForward for QuantizedQwen2ForCausalLM {
    fn forward(
        &self,
        input_ids: &Tensor,
        seqlen_offset: usize,
...
mod tests {
    ...
    #[test]
    fn test_quantized_qwen2_construction() {
        let cfg = test_config();
        ...
        let model = QuantizedQwen2ForCausalLM::new(&cfg, vb, loader.as_ref());
        assert!(
            model.is_ok(),
            "QuantizedQwen2ForCausalLM should construct with unquantized loader"
        );
    ...
    #[test]
    fn test_quantized_qwen2_forward_shape() {
        let cfg = test_config();
        let device = Device::Cpu;
        let vb = candle_nn::VarBuilder::zeros(DType::F32, &device);
        ...
        let detected = DetectedQuantConfig::default();
        let loader = create_weight_loader_with_params(vb.clone(), &detected);
        let model = QuantizedQwen2ForCausalLM::new(&cfg, vb, loader.as_ref()).expect("build model");
