...
- ~~**Registry completeness audit (round 2)**~~ ✅ DONE (commit 22fce8b): systematic Python registry vs Rust registry comparison; added Llama_Nemotron_Nano_VL + NemotronParseForConditionalGeneration as new entries; added NemotronNasForCausalLM + MiniCPMOForCausalLM + Grok2ForCausalLM + StablelmForCausalLM as secondary aliases; EagleDeepseekV3ForCausalLM + ERNIE aliases from prior commit 22d8a8b.
- ~~**BEE**~~ ✅ DONE (`llava_onevision.rs`, 3+2 tests — commit e11a49a): `BeeForConditionalGeneration` with `BeeMultiModalProjector` (LayerNorm + Linear[vis→text*4] + GELU + Linear[text*4→text]). Weight paths: `multi_modal_projector.{pre_norm,linear_1,linear_2}`.
- ~~**RVL**~~ ✅ DONE (`llava_onevision.rs`, 3+1 tests — commit e11a49a): `RForConditionalGeneration` with `RvlMultiModalProjector` (LayerNorm + Linear[vis→text] + GELU + Linear[text→text]). Same weight paths.
- ~~**TeleFLM**~~ ✅ DONE (`llama.rs`, 4+2 tests — commit e11a49a): `TeleFLMForCausalLM` wraps `LlamaForCausalLM` with optional µP: embeddings×input_mult + logits×(output_mult/mup_scale_factor). Config: `use_mup`, `input_mult`, `output_mult`, `mup_scale_factor` in extra.
- **P2 models (~1 remaining):** ~~MiniCPM-O~~ ✅ DONE (`minicpmo.rs`, 5 tests — commit 7763a99), MiniMax-VL-01 (BLOCKED: Lightning Attention), ~~Nemotron-VL~~ ✅ DONE (`nemotron_vl.rs`, 5 tests — commit 7129c07 — RadioModel flat vision_config + LayerNorm+GELU projector + LlamaForCausalLM), ~~Hunyuan-Vision~~ ✅ DONE (`hunyuan_vision.rs`, 5 tests + XDRoPE — commit 94ada76), ~~LFM2-VL~~ ✅ DONE (`lfm2_vl.rs`, 6 tests — commit f0ac8f7 — Siglip2VisionTransformer + pixel-shuffle projector + Lfm2ForCausalLM)
